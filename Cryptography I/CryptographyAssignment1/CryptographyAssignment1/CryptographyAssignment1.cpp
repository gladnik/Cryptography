...
unsigned char GetKeyLetter(int pos)
{
	string ct[11];
	unsigned char ctChar[11], keyLetter = 255, ctXor[11][11] = {0};
	...
	for (int i = 0; i < 11; i++)
	{
		for (int j = 0; j < 11; j++)
		{
			if (i != j)
			{
				ctXor[i][j] = ctChar[i] ^ ctChar[j]; //Then xor these characters with one another.
			}
		}
	}
	bool valid = true;
	for (int i = 0; i < 11; i++)  //A character from [a-zA-Z] xored with a space is just the same character in the inverted case.
	{
		...
		if (valid) //If a space in some message convert other messages to [a-zA-Z].
			keyLetter = ctChar[i] ^ (int)' '; //We can use that message to get a letter of the key.
	}
...
int main()
{
	...
	for (int pos = 0; pos < (int)(ciphertext[10].length() / 2); pos++) //Then decrypt letter by letter.
	{
		...
		{		
			...
			{
				msg[ctNumber].append(1, '_'); //Replace that character with a '_'
				continue;
			} //If we found a key
			msgChar = GetCtChar(ctNumber, pos) ^ keyLetter; //decrypt
			msg[ctNumber].append(1, msgChar); //and reconstruct the original message.
		}
	}
	for (int i = 0; i < 11; i++)
	{
		cout<<msg[i]<<endl; //Although some characters will be replaced with '_', we can easily get original messages from semantics.
	}
	return 0;
}